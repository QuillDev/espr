...
class ItemBuilder {
    ...
    fun displayName(displayName: Component): ItemBuilder {
        meta()?.also { it.displayName(displayName.decoration(TextDecoration.ITALIC, false)); meta(it) }
        return this
    }
    ...
    fun lore(lore: List<Component>): ItemBuilder {
        meta()?.also { it.lore(lore.map { line -> line.decoration(TextDecoration.ITALIC, false) }); meta(it) }
        return this
    }
